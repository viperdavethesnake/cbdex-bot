...
**Status:** Active — supersedes v1.3  
**Updated:** 2026-04-14  
**Changes from v1.3:** Pool addresses locked; GeckoTerminal designated sole Fast Path source; DexScreener removed; TVL resolution corrected to hourly forward-fill; Coinbase execution path corrected to direct Aerodrome Router; min signal thresholds updated; pool-type schemas documented; subgraph sync check added as pre-flight requirement.
...
Compare the returned block number against the current Base chain head. If the delta exceeds 1,000 blocks (~33 minutes), the subgraph is stale. **Do not proceed with Truth Path queries against a stale subgraph.** Escalate to: Bitquery Aerodrome API, a newer community subgraph, or direct RPC log parsing via `eth_getLogs`.

---

### Step 1: Bulk Ingestion (Fast Path)

**Source: GeckoTerminal only.** DexScreener does not provide historical OHLCV at any resolution and cannot serve the Fast Path role.
...
For the three selected windows, pull raw `Swap` events via The Graph (GraphQL) and block headers for gas via `eth_getBlockByNumber`. TVL sourced from `PoolHourData`.

**Pagination:** All GraphQL queries must use `id_gt` cursor-based pagination. The `skip` parameter has a hard ceiling of 5,000 records and will silently fail on high-volume pairs.

**Pool-type-specific field names must be verified via schema introspection before writing production queries.**
...
|---|---|---|
| **Price Correlation (ρ)** | > 0.999 | Pearson correlation of Close prices |
| **MAE** | < 0.10% | Mean Absolute Error — accounts for same-second ordering noise |
| **Volume Error** | < 1% | \|Vol_fast - Vol_truth\| / Vol_truth |
| **TVL Error** | null | Always null — no Fast Path TVL. Not a fail condition. |
| **Dropped Candles** | 0 | Minutes where Truth has activity but Fast Path is empty |
| **Filled Candles** | Info Only | Forward-filled empty windows — logged, not gated |
  ...
  "windows": [
    {
      "regime": "spike",
      "date": "2026-01-14",
      "price_correlation": 0.9994,
      "mae_pct": 0.07,
      "volume_error_pct": 0.41,
      "tvl_error_pct": null,
      "gap_count_dropped": 0,
      "gap_count_filled": 3,
      "pass": true
...
If `overall_verdict: FAIL`: abandon GeckoTerminal, execute full 90-day Truth Path pull via The Graph. Expect 1,000+ paginated queries. Budget 8–20 hours. All queries must use `id_gt` cursor pagination.
